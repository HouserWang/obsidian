...
- 标题：**[[深入理解 Java NIO 模型在 RocketMQ HA 中的实践]]**
    
- 感悟：**“OP_WRITE 不是写完成事件，而是可写（缓冲区不满）事件。这是实现高效异步传输的关键。”** —— 这句话能秒杀 80% 的 Java 面试者。
  
  
  “在分析 RocketMQ 的 HA 源码时，我发现它在 ReadSocketService 中并没有采用标准的 Reactor 多路复用分发模式，而是为每个 Slave 连接分配了独立的 Selector 线程。

这种设计的巧妙之处在于：它利用 select(1000) 的超时机制兼顾了**事件驱动**与**心跳检测**。由于连接具有唯一性，它避开了复杂的 SelectionKey 迭代过程，直接在唤醒后进行非阻塞读取。

这让我认识到：**架构设计不应拘泥于模式，在连接数极少且对实时性要求极高的场景下，这种‘一线程一连接’的 NIO 变种实现，往往比复杂的通用模式更高效且易于维护。**”


NeedLock=(Write∩Shared)∧(Dependency∨Composite)

1. **Write ∩ Shared（有写且共享）**：如果只是读，或者数据只有我一个人用（ThreadLocal），不需要锁。
2. **Dependency（依赖）**：
    - 我的这次写入，依赖于**“旧值”**吗？（如 a++，余额扣减）
    - 我的这次写入，依赖于**“前置条件”**吗？（如 状态机必须是 UNPAID 才能改成 PAID）
3. **Composite（组合/复合）**：
    - 我这次修改涉及**多个变量/多张表**，必须同时成功或同时失败，不能暴露出中间态吗？