...
- **痛点描述**：缓存一致性，缓存的穿透，击穿，雪崩、代码复杂度
- **解决方案**：使用jetcache框架操作缓存，它内部使用的是cache aside模式并可以自定义延迟双删来保证一致性，可以设置空值缓存和布隆过滤器来解决缓存穿透，可以设置在缓存查不到查数据库的时候使用分布式锁来解决缓存击穿，可以设置不同的过期时间来解决缓存雪崩。并且就像使用spring cache一样注解式的使用缓存，对业务代码无侵入。在库存这些需要自己操作缓存的场景，它对缓存组件进行了良好的封装，可以优雅的编程式使用。
- **收益/价值**：缓存带来最大好处是应用请求的响应时间大幅减少，并且可以减轻数据库压力。而库存这些高并发场景，缓存可以扛住高并发，给用户良好的体验

## 4. 简历话术预案 (Resume Snippet)
> 📝 **降维打击**：明年写简历时，直接复制这一段。
> *格式建议：掌握 [技术原理]，在 [业务场景] 中，通过 [技术手段]，解决了 [什么问题/提升了什么指标]。*

- **话术 1 (侧重原理)**：深入理解 **redis reactor/网络多线程io模型，redis sentinal/cluster架构、redis持久化** 原理，能从内核层面分析redis网络问题，性能问题。
- **话术 2 (侧重实战)**：在 **c端高并发** 业务场景中，设计了 **redis缓存** 方案，解决了 **bigkey导致redis性能抖动** 问题。
